package com.nxt.net.baseapp.app.utils;
...
import com.cauc.mavenj.interpolators.BackOut;
import com.cauc.mavenj.interpolators.CircInOut;
import com.cauc.mavenj.interpolators.ExpoIn;
import com.cauc.mavenj.interpolators.ExpoOut;
import com.cauc.mavenj.interpolators.QuadInOut;
import com.cauc.mavenj.interpolators.QuintInOut;
import com.cauc.mavenj.interpolators.QuintOut;
import com.nxt.net.baseapp.R;


public class TransitionHelper {
    private static final String TAG = "TransitionNoDepthHelper";
    ...
    public static RelativeLayout getLayout(View root, int idRes) {
        return (RelativeLayout) root.findViewById(idRes);
    }

    public static void startIntroAnim(View root, AnimatorListenerAdapter introEndListener) {
        RelativeLayout dl = getLayout(root, R.id.root_dl);
        ObjectAnimator objectAnimator = null;
        if (dl != null) {
            objectAnimator = introAnimate(dl, 0, 30f, 15, 180);
        }
        dl = getLayout(root, R.id.dl0);
        if (dl != null) {
    ...
    static ObjectAnimator introAnimate(final RelativeLayout target, final float moveY, final float customElevation, long delay, int subtractDelay) {
        Log.e(TAG, "introAnimate: rootView.getTranslationY() = " + target.getTranslationY());
        ...
        translationY2.addListener(new AnimatorListenerAdapter() {
            ...
            public void onAnimationEnd(Animator animation) {
                ...
                Log.e(TAG, "onAnimationEnd: end y = " + (-moveY * target.getResources().getDisplayMetrics().density));
            }
        });
...
//        target.setCustomShadowElevation(customElevation * target.getResources().getDisplayMetrics().density);
    ...
    public static void startExitAnim(View root) {
        exitAnimate((RelativeLayout) root.findViewById(R.id.root_dl), 0, 30f, 15, 190, true);
        exitAnimate((RelativeLayout) root.findViewById(R.id.dl0), MOVE_Y_STEP, 20f, 60, 230, true);
        exitAnimate((RelativeLayout) root.findViewById(R.id.dl2), MOVE_Y_STEP, 20f, 60, 230, true);
        hideStatusBar(root);
    }
...
//        decorView.setSystemUiVisibility(uiOptions);
    ...
    static ValueAnimator exitAnimate(final RelativeLayout target, final float moveY, final float customElevation, long delay, int subtractDelay, boolean continueOffscreen) {
...
//        elevation.start();
    ...
    private static void continueOutToRight(RelativeLayout target, float moveY, int subtractDelay) {
...
//        translationY.start();
    ...
    static ObjectAnimator revertFromMenu(final RelativeLayout target, final float customElevation, int subtractDelay, float targetElevation) {
...
//        target.setCustomShadowElevation(customElevation * target.getResources().getDisplayMetrics().density);
    ...
    @NonNull
    private static AnimatorListenerAdapter getShowStatusBarListener(final RelativeLayout target) {
